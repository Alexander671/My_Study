...
class Stack: # - для управления односвязным списком.
    ...
    def __getitem__(self,key):
        self.__validate(key) # проверка
        return self.get_data()[key] # вернули через вспомогательный метод get_item
    ...
    def __setitem__(self, key, value):
        ...
        if self.top == None:
            ...
            i = 0
            iterate = self.top            # начинаем с самого начала
            while i != key:        # если значение уже последнее, либо это начало, либо нашли последний
                i += 1
                iterate = iterate.get_next() # двигаемся по списку, если уткнулись в последний, то изменяем его
            else:
                iterate.data = value.data                 # если изначально был None, то изменим просто top
