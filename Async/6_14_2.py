...
async def check_message(message):
    ...
    words = remove_punctuation(message['message']).split()

    # проверили, что есть хотя бы одно вхождение
    # запрещенного слова
    if any(map(lambda x: x.lower() in banned_words, words)):
    
        # получили текущее задание, вывели сообщение
...
async def main():
    tasks = map(asyncio.create_task, map(check_message, message))
    try:
