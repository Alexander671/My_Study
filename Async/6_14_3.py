...
async def check_message(message):
    ...
    # разделили на список слов
    message = message['message']
    words = remove_punctuation(message).split()
    ...
    elif task_name == 'black_list_user':
    ...
    for banned_word in banned_words:
        if banned_word in message.lower():
            ...
            elif task_name == 'moderator':
                print(f"{task_name}: {message.replace(banned_word, '****')}")

            elif task_name == 'student':
        ...
        # вывели сообщение, если нет запрещенных слов
    else:
        print(f"{task_name}: {message}")
...
async def main():
    for message in messages:
        task = asyncio.create_task(check_message(message), name=message['role'])
        task.set_name(message['role'])
    try:
