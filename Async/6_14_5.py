...
async def bus_boarding(passanger):
    asyncio.current_task().name = passanger['Name']
    asyncio.current_task().job = passanger['Job']

    await asyncio.sleep(passanger['Speed'])
    print(f"{passanger['Name']} сел в автобус.")

async def main():
    tasks = [asyncio.create_task(bus_boarding(passenger)) for passenger in passengers]
    try:
        ...
        for task in tasks:
            if task.cancelled():    
                print(f'{task.name} {task.job} не успел/а вовремя сесть в автобус.')
